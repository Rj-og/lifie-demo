 ...
 */
export async function fetchPreset(targetPresetName = 'AI SDR') {
  ...
  const targetPreset = presets.find(
    (preset) => preset.presetName === targetPresetName
  );

  if (!targetPreset) {
    console.error('Available presets:', JSON.stringify(presets, null, 2));
    throw new Error(
      `Preset "${targetPresetName}" not found. Available: ${presets.map(p => p.presetName).join(', ')}`
    );
  }
...
export async function executeDemoRequestFlow(leadData) {
  // Step 1: Fetch the AI SDR preset
  console.log('[Lifie API] Step 1: Fetching presets...');
  const preset = await fetchPreset('AI SDR');
  console.log('[Lifie API] Step 1 complete. Preset found:', preset.presetName);
